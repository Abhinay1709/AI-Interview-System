...
# matplotlib is only needed to generate charts embedded into the report.
# The app should still work (downloadable DOCX) even if matplotlib isn't installed.
try:
...
def _add_section_heading(doc, text, level=1):
    """Bold section heading with consistent styling."""
    para = doc.add_paragraph()
...
def _build_summary_table(doc, headers, values,
    ...
    """Create a 2-row table (header + value)."""
    n = len(headers)
...
def generate_score_chart(technical, communication, confidence, overall):
    """Generate a bar chart image for the report.

    Returns:
        str | None: chart file path, or None if matplotlib isn't installed.
    """
    if plt is None:
        return None
    ...
    values = [technical, communication, confidence, overall]
...
def generate_full_report(questions, answers, evaluation):
    ...
    run = cover.add_run("AI INTERVIEW COACH")
    run.bold = True
    run.font.size = Pt(26)

    subtitle = doc.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    subtitle_run = subtitle.add_run("Professional Interview Assessment Report")
    ...
    info_table.cell(1, 1).text = datetime.now().strftime("%d %B %Y")

    overall_score = safe_extract(r"Overall Score:\s*([0-9]+(?:\/10)?)", evaluation, "N/A")
    technical_score = safe_extract(r"Technical Score:\s*([0-9]+(?:\/10)?)", evaluation, "N/A")
    ...
    info_table.cell(4, 1).text = "Placement Ready"
    ...
    footer.add_run("Generated by AI Interview Coach")

    _add_horizontal_rule(doc, color="1F497D")

    # EXECUTIVE SUMMARY
    _add_section_heading(doc, "EXECUTIVE SUMMARY")
    overall_score_num = safe_extract(r"Overall Score:\s*(\d+)", evaluation, "0")
    try:
        score = int(overall_score_num)
    except Exception:
        score = 0
    ...
    _build_summary_table(
        doc,
        headers=["Technical Score", "Communication Score", "Confidence Score", "Overall Score"],
        values=[technical_score, communication_score, confidence_score, overall_score],
    ...
    if chart:
        doc.add_picture(chart, width=Inches(4.5))
    ...
    attempted_match = re.search(r"Questions Attempted:\s*(\d+)\/(\d+)", evaluation, re.IGNORECASE)
    skipped_match = re.search(r"Questions Skipped:\s*(\d+)\/(\d+)", evaluation, re.IGNORECASE)
    ...
    if not questions:
        ...
        for idx, question in enumerate(questions, start=1):
            ...
            _add_block_content(doc, "Model Answer", model_answer, label_color=COLOR_MID_BLUE)
            _add_block_content(doc, "Feedback", feedback, label_color=COLOR_ORANGE)
    ...
    strengths = safe_extract(r"Strengths:(.*?)Weaknesses:", evaluation, "Not Available")
    ...
    weaknesses = safe_extract(r"Weaknesses:(.*?)Suggestions:", evaluation, "Not Available")
    _add_bullet_points(doc, weaknesses)
    _add_horizontal_rule(doc, color="1F497D")
    _spacer(doc, before=6, after=4)

    # READINESS ASSESSMENT
    _add_section_heading(doc, "READINESS ASSESSMENT")

    try:
        overall_num = int(overall_score.split("/")[0])
    except Exception:
        overall_num = 0

    readiness = overall_num * 10
    _add_label_value(doc, "Readiness Score", f"{readiness}%")

    if readiness >= 80:
        doc.add_paragraph("Placement Ready")
    elif readiness >= 60:
        doc.add_paragraph("Almost Ready")
    else:
        doc.add_paragraph("Needs More Preparation")

    _add_horizontal_rule(doc)

    # FINAL RECOMMENDATION
    _add_section_heading(doc, "FINAL RECOMMENDATION")
    if readiness >= 80:
        recommendation = "Candidate is ready for interview opportunities."
    elif readiness >= 60:
        recommendation = "Candidate should improve weak areas before attending interviews."
    else:
        recommendation = "Candidate should focus on fundamentals and practice interviews."
    ...
    suggestions = safe_extract(r"Suggestions:(.*)", evaluation, "Not Available")
    ...
    fr = fp.add_run("AI Interview Preparation System  |  Abhinay Andhavarapu")
    fr.font.size = Pt(9)
